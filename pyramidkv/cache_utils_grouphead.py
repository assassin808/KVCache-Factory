...
class DynamicCache(Cache):
    ...
    def __init__(self, config: PretrainedConfig = None) -> None:
      ...
      self.compressed_key_cache: List[torch.Tensor] = []
      self.compressed_value_cache: List[torch.Tensor] = []
      self.key_scale: List[torch.Tensor] = []

      self._seen_tokens = 0  # Used in `generate` to keep tally of how many tokens the cache has seen

      # For assisted decoding and profiling
    ...
    def from_legacy_cache(cls, past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None):
        ...
        if past_key_values is not None:
            for layer_idx in range(len(past_key_values)):
                ...
                cache.compressed_key_cache.append(past_key_values[layer_idx][2])
                cache.compressed_value_cache.append(past_key_values[layer_idx][3])
                cache.key_scale.append(past_key_values[layer_idx][4])
    ...
    def to_legacy_cache(self) -> Tuple[Tuple[torch.Tensor], Tuple[torch.Tensor]]:
        ...
        legacy_cache = ()
        for layer_idx in range(len(self)):
            legacy_cache += ((self.retained_key_cache[layer_idx],  self.retained_value_cache[layer_idx], self.compressed_key_cache[layer_idx], self.compressed_value_cache[layer_idx], self.key_scale[layer_idx],),)
        return legacy_cache
