...
class DynamicCache(Cache):
    ...
    def update_miniCache(
        ...
        if layer_idx == num_layers - 1:
            ...
            import numpy as np
            print_tensor_summary('key', calculate_interlayer_similarity(self.retained_value_cache))
            similarity_matrix_np = calculate_interlayer_similarity(self.retained_value_cache).cpu().numpy()
            np.savetxt("similarity_matrix_value.csv", similarity_matrix_np, delimiter=",")
            exit(0)
    ...
    def update_miniCache_decode(self,
        ...
        if layer_idx < num_layers//8 or layer_idx % 2 == 1:
