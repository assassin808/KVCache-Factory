...
from pyramidkv.pyramidkv_utils import init_pyramidkv,init_snapkv,init_CAM,init_H2O,init_StreamingLLM,init_l2norm, init_adakv, init_headkv, init_MiniCacheKV, init_3D
import math
...
def llama_attn_forward_MiniCache(
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len:
            self.kv_seq_len = kv_seq_len
            key_states, value_states, hidden_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs, hidden_states, attention_mask)
    ...
    if key_states.shape[-2] == kv_seq_len:
        past_key_value.store_attn_output(attn_output,query_states)
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if self.layer_idx == 0:
            ...
            prev_key_states, prev_query_states, prev_value_states, prev_hidden_states = past_key_value.key_cache[self.layer_idx-1], None, past_key_value.value_cache[self.layer_idx-1]
