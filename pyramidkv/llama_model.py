...
def llama_attn_forward_H2O(
    ...
    **kwargs,
    ...
    if key_states.shape[0] != 1:
        ...
        query_states_compress =  query_states[1:2,:,:,:]
        query_states =  query_states[1:2,:,:,:]
    else:
        query_states_compress = query_states

    attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim)
    if past_key_value is not None and key_states.shape[-2] == kv_seq_len:
        attn_weights_compress = torch.matmul(query_states_compress, key_states_compress.transpose(2, 3)) / math.sqrt(self.head_dim)
    ...
    if past_key_value is not None and key_states.shape[-2] == kv_seq_len and self.layer_idx != 31:
        # return attn_output, attn_weights, past_key_value
        return torch.concat((attn_output,attn_output_compress),dim=0), attn_weights, past_key_value
    else:
        return attn_output, attn_weights, past_key_value
